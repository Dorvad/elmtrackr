...
class LegacyDataAdopter @Inject constructor(
    ...
    suspend fun adoptFor(userId: String) {
        ...
        database.withTransaction {
            ...
            database.projectDao().adoptLegacyUser(userId)
            database.projectBillingRecordDao().adoptLegacyUser(userId)
            database.projectPaymentDao().adoptLegacyUser(userId)
            database.workplaceDao().adoptLegacyUser(userId)
            database.leavePolicyDao().adoptLegacyUser(userId)
            database.absenceEventDao().adoptLegacyUser(userId)
            database.absenceAllocationDao().adoptLegacyUser(userId)
            database.leaveBalanceSnapshotDao().adoptLegacyUser(userId)
