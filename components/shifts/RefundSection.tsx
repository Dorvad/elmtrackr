...
const SCOOTER_THEME = {
  ...
  bird: {
    frame: "#D8D9DC", outline: "#AAABAF", strip: "#00A3FF",
    box: "#00A3FF", boxMark: "rgba(255,255,255,.9)",
    tire: "#1C1C21", rim: "#38383F", spoke: "#5C5C64", hub: "#AAABAF",
    grip: "#1C1C21", light: "#00A3FF",
  },
} as const;

type ScooterBrand = "lime" | "dott" | "bird";
...
const SCENE = {
  ...
  bird: {
    skyTop: "#E4F4FC", skyBot: "#C4E2F0",
    sun: "radial-gradient(circle, rgba(0,163,255,.75), rgba(0,163,255,0) 70%)",
    hillFar: "#A0C8E4", hillNear: "#78AEC8",
    road: "linear-gradient(180deg,#5EA8C8,#448EB0)",
  },
} as const;
    ...
    <div className="ride-scene" style={{ position: "relative", height: 160, overflow: "hidden",
      ...
      <div style={{ position: "absolute", top: 12, left: 14, zIndex: 6,
        ...
        <span style={{ fontFamily: "var(--au-display)", fontWeight: 700, fontSize: 13.5,
          letterSpacing: "-.01em",
          color: brand === "lime" ? "#1E8A0E" : brand === "dott" ? "#1B4FA8" : "#006BBF" }}>
          {brand === "lime" ? "Lime" : brand === "dott" ? "dott" : "Bird"}
        </span>
...
// ── Taxi illustration ─────────────────────────────────────────────────────────
function TaxiCarSVG({ wheelDur }: { wheelDur: string }) {
  function CarWheel({ cx, cy }: { cx: number; cy: number }) {
    const spokes = [0, 1, 2, 3].map(i => {
      const a = i * 45 * Math.PI / 180;
      return { x1: cx - 11 * Math.cos(a), y1: cy - 11 * Math.sin(a), x2: cx + 11 * Math.cos(a), y2: cy + 11 * Math.sin(a) };
    });
    return (
      <>
        <circle cx={cx} cy={cy} r={20} fill="#1A1A1F" />
        <g style={{ transformOrigin: `${cx}px ${cy}px`, animation: `rideWheelSpin ${wheelDur} linear infinite` }}>
          <circle cx={cx} cy={cy} r={12} fill="#3A3A42" />
          {spokes.map((s, i) => (
            <line key={i} x1={s.x1} y1={s.y1} x2={s.x2} y2={s.y2}
              stroke="#606068" strokeWidth={2} strokeLinecap="round" />
          ))}
          <circle cx={cx} cy={cy} r={3.5} fill="#9898A0" />
        </g>
      </>
    );
  }
  ...
  return (
    <svg viewBox="0 0 220 126" aria-hidden
      style={{ display: "block", height: 112, width: "auto", overflow: "visible",
        filter: "drop-shadow(0 7px 8px rgba(20,16,48,.20))" }}>
      {/* Lower body */}
      <rect x={20} y={58} width={180} height={40} rx={10} fill="white" stroke="#D4D4D8" strokeWidth={1.5} />
      {/* Cabin */}
      <path d="M58,58 L73,20 L147,20 L162,58 Z" fill="white" stroke="#D4D4D8" strokeWidth={1.5} />
      {/* Rear window */}
      <path d="M77,56 L84,24 L113,24 L113,56 Z" fill="#AACCE0" fillOpacity={0.88} />
      {/* Front window */}
      <path d="M117,56 L117,24 L144,24 L153,56 Z" fill="#AACCE0" fillOpacity={0.88} />
      {/* B-pillar */}
      <rect x={113} y={22} width={4} height={34} fill="white" />
      {/* Door line */}
      <line x1={115} y1={58} x2={115} y2={96} stroke="#D8D8DC" strokeWidth={1} />
      {/* Door handle */}
      <rect x={120} y={74} width={14} height={4} rx={2} fill="#C8C8CC" />
      {/* Wheel arches */}
      <path d="M20,78 Q40,62 60,78" fill="white" stroke="#D4D4D8" strokeWidth={1.5} />
      <path d="M142,78 Q162,62 182,78" fill="white" stroke="#D4D4D8" strokeWidth={1.5} />
      {/* Headlight */}
      <rect x={192} y={64} width={10} height={10} rx={2.5} fill="#FFFACC" stroke="#E8D860" strokeWidth={1} />
      {/* Tail light */}
      <rect x={18} y={64} width={10} height={10} rx={2.5} fill="#FFAAAA" stroke="#E08080" strokeWidth={1} />
      {/* Front bumper */}
      <rect x={192} y={76} width={12} height={8} rx={3} fill="#E0E0E4" stroke="#CCCCCE" strokeWidth={1} />
      {/* Rear bumper */}
      <rect x={16} y={76} width={12} height={8} rx={3} fill="#E0E0E4" stroke="#CCCCCE" strokeWidth={1} />
      {/* Wheels */}
      <CarWheel cx={58} cy={96} />
      <CarWheel cx={162} cy={96} />
      {/* Taxi roof sign */}
      <rect x={80} y={5} width={60} height={20} rx={5} fill="#FFD000" />
      <text x={110} y={19.5} textAnchor="middle" fill="#2A2000" fontWeight={800} fontSize={10.5}
        fontFamily="sans-serif" letterSpacing={0.8}>TAXI</text>
      {/* Israeli license plate */}
      <rect x={148} y={82} width={36} height={14} rx={3} fill="#FFD000" stroke="#C8A000" strokeWidth={1} />
    </svg>
  );
}

function TaxiScene({ isSelected }: { isSelected: boolean }) {
  const farDur   = isSelected ? "18s" : "30s";
  const nearDur  = isSelected ? "10s" : "17s";
  const dashDur  = isSelected ? "0.65s" : "1.05s";
  const wheelDur = isSelected ? "0.45s" : "0.9s";
  const bobDur   = isSelected ? "2.8s" : "3.5s";
  const streaks  = isSelected ? ["0.85s", "0.65s", "1.05s"] : ["1.4s", "1.1s", "1.7s"];

  const farH  = [42, 58, 34, 70, 28, 52, 46, 62, 38, 54];
  const nearH = [55, 72, 44, 84, 36, 66, 50];

  return (
    <div className="ride-scene" style={{ position: "relative", height: 160, overflow: "hidden",
      filter: isSelected ? undefined : "saturate(.85)" }}>

      {/* Urban sky */}
      <div style={{ position: "absolute", inset: 0,
        background: "linear-gradient(180deg,#C4CAD6 0%,#B2BAC8 100%)" }} />

      {/* Moon / hazy sun */}
      <div style={{ position: "absolute", top: 16, right: 36, width: 38, height: 38,
        borderRadius: "50%", background: "radial-gradient(circle, rgba(255,240,180,.7), rgba(255,240,180,0) 70%)",
        filter: "blur(1px)", animation: "rideSunPulse 6s ease-in-out infinite" }} />

      {/* Far building strip */}
      <div style={{ position: "absolute", left: 0, bottom: 44, display: "flex",
        width: "200%", height: 90, animation: `rideScrollX ${farDur} linear infinite`, willChange: "transform",
        alignItems: "flex-end" }}>
        {[0, 1].map(t => (
          <div key={t} style={{ width: "50%", flexShrink: 0, height: "100%",
            display: "flex", alignItems: "flex-end", gap: 3, paddingLeft: 4 }}>
            {farH.map((h, i) => (
              <div key={i} style={{ width: 16 + (i % 4) * 8, height: h, background: "#7A8298",
      ...
      {/* Near building strip */}
      <div style={{ position: "absolute", left: 0, bottom: 44, display: "flex",
        width: "200%", height: 68, animation: `rideScrollX ${nearDur} linear infinite`, willChange: "transform",
        alignItems: "flex-end" }}>
        {[0, 1].map(t => (
          <div key={t} style={{ width: "50%", flexShrink: 0, height: "100%",
            display: "flex", alignItems: "flex-end", gap: 4, paddingLeft: 6 }}>
            {nearH.map((h, i) => (
              <div key={i} style={{ width: 22 + (i % 3) * 14, height: h, background: "#545E72",
                borderRadius: "2px 2px 0 0", flexShrink: 0 }} />
            ))}
          </div>
        ))}
      ...
      {/* Speed streaks */}
      <div style={{ position: "absolute", bottom: 52, left: 0, width: "100%", height: 68,
        pointerEvents: "none", opacity: isSelected ? 0.9 : 0.45 }}>
        {[{top: 6, width: 56}, {top: 28, width: 72}, {top: 48, width: 42}].map((s, i) => (
          <div key={i} style={{ position: "absolute", top: s.top, width: s.width, height: 3,
            borderRadius: 3, right: -60, opacity: 0,
            background: "linear-gradient(90deg, transparent, rgba(255,255,255,.75))",
            animation: `rideStreak ${streaks[i]} linear infinite`, animationDelay: `${i * 0.42}s` }} />
        ))}
      </div>

      {/* Shadow */}
      <div style={{ position: "absolute", bottom: 16, left: "50%", width: 140, height: 14,
        borderRadius: "50%", zIndex: 3, willChange: "transform, opacity",
        background: "radial-gradient(50% 50%, rgba(20,16,48,.24), rgba(20,16,48,0) 72%)",
        animation: `rideShadow ${bobDur} ease-in-out infinite` }} />

      {/* Taxi car */}
      <div style={{ position: "absolute", bottom: 20, left: "50%", zIndex: 4, willChange: "transform",
        animation: `rideBob ${bobDur} ease-in-out infinite` }}>
        <TaxiCarSVG wheelDur={wheelDur} />
      </div>

      {/* Road */}
      <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, height: 44, background: "#38404E" }}>
        <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3,
          background: "rgba(255,255,255,.25)" }} />
        <div style={{ position: "absolute", top: 18, left: 0, height: 5, width: "200%",
          borderRadius: 3, willChange: "transform",
          background: "repeating-linear-gradient(90deg, rgba(255,255,255,.88) 0 26px, transparent 26px 52px)",
          animation: `rideScrollDash ${dashDur} linear infinite` }} />
      </div>
      ...
      <div style={{ position: "absolute", top: 12, left: 14, zIndex: 6,
        ...
        boxShadow: "0 6px 18px -8px rgba(20,16,48,.3)" }}>
        <span style={{ fontFamily: "var(--au-display)", fontWeight: 700, fontSize: 13.5,
          letterSpacing: "-.01em", color: "#B88A00" }}>Taxi</span>
        <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: ".04em",
          color: "var(--au-ink-2)", whiteSpace: "nowrap" }}>private car</span>
      </div>
    </div>
  );
}

function TaxiCard({ isSelected, onClick }: { isSelected: boolean; onClick: () => void }) {
  const borderColor = isSelected ? "#D4A000" : "transparent";
  const checkBg     = isSelected ? "#D4A000" : "white";
    ...
    <button type="button" onClick={onClick}
      ...
      style={{
        ...
        boxShadow: isSelected
          ? "0 22px 52px -22px rgba(180,140,0,.44), 0 2px 8px rgba(180,140,0,.06)"
          : "0 10px 28px -18px rgba(80,64,210,.28), 0 1px 3px rgba(80,64,210,.05)",
        transition: "box-shadow .28s, border-color .28s",
      }}>
      <TaxiScene isSelected={isSelected} />
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px" }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 15, fontWeight: 800, color: "var(--au-ink)" }}>Taxi</div>
          <div style={{ fontSize: 12, color: "var(--au-faint)", marginTop: 2 }}>Private taxi ride</div>
        </div>
...
function ScooterCard({
  ...
  const name        = brand === "lime" ? "Lime" : brand === "dott" ? "Dott" : "Bird";
  const accent      = brand === "lime" ? "#3FC81E" : brand === "dott" ? "#2E6FD6" : "#00A3FF";
  const borderColor = isSelected ? accent : "transparent";
  const checkBg     = isSelected ? accent : "white";
  const checkBorder = isSelected ? checkBg : "var(--au-hair)";
  const shadowColor = brand === "lime" ? "30,138,14" : brand === "dott" ? "27,79,168" : "0,130,220";
  ...
  return (
    <button type="button" onClick={onClick}
      ...
      style={{
        ...
        background: "var(--au-surface)",
        boxShadow: isSelected
          ? `0 22px 52px -22px rgba(${shadowColor},.44), 0 2px 8px rgba(${shadowColor},.06)`
          : "0 10px 28px -18px rgba(80,64,210,.28), 0 1px 3px rgba(80,64,210,.05)",
        transition: "box-shadow .28s, border-color .28s",
      }}>
...
function ClaimCard({
  ...
  const ctaLabel = (() => {
    ...
    if (provider === "Bird") return `Add ₪${amt.toFixed(2)} Bird refund`;
    if (provider === "Taxi") return `Add ₪${amt.toFixed(2)} Taxi refund`;
  ...
  const ctaGradient = provider === "Lime"
    ...
    : provider === "Bird"
    ? "linear-gradient(118deg,#006BBF,#00A3FF)"
    : provider === "Taxi"
    ? "linear-gradient(118deg,#B87800,#E5A000)"
    : "var(--au-grad)";
  const ctaShadow = provider === "Lime"
    ...
    : provider === "Bird"
    ? "0 14px 28px -12px rgba(0,130,210,.52)"
    : provider === "Taxi"
    ? "0 14px 28px -12px rgba(180,120,0,.52)"
      ...
      {showForm && (
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            <ScooterCard brand="bird" isSelected={provider === "Bird"}
              onClick={() => setProvider("Bird")} />
            <TaxiCard isSelected={provider === "Taxi"}
              onClick={() => setProvider("Taxi")} />
